...
const HomePage = () => {
  ...
  // Fonctions pour vérifier si un groupe ou un champ a des conversations associées
  // Définies comme des fonctions simples pour éviter les dépendances circulaires
  ...
  const MessageItem = React.memo(({ message }: { message: Message }) => {
    ...
    return (
      <div className={`flex mb-4 ${isCurrentUser ? 'justify-end' : 'justify-start'}`}>
        <div className={`max-w-3/4 rounded-lg px-4 py-2 ${isCurrentUser ? 'bg-indigo-100 text-indigo-900' : 'bg-gray-100 text-gray-900'}`}>
  ...
  const ConversationSidebar = () => {
    ...
    return (
      <div className={`fixed right-0 top-0 h-full w-96 bg-white border-l border-gray-200 shadow-lg transform transition-transform duration-300 ease-in-out z-20 ${sidebarOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        <div className="flex flex-col h-full" onClick={(e) => e.stopPropagation()}>
          {/* Header */}
          <div className="px-4 py-5 border-b border-gray-200 flex justify-between items-center bg-indigo-600 text-white">
            <h2 className="text-xl font-semibold">
              {viewMode === 'selection' ? 'Sélection' : 'Conversation'}
            </h2>
  ...
  return (
    <div className="relative max-w-7xl mx-auto px-4 py-10 bg-gray-50 min-h-screen">
      {/* Bouton d'ouverture du panneau latéral */}
      <button
        className={`fixed z-30 right-6 bottom-6 bg-indigo-600 text-white rounded-full p-3 shadow-lg hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 ${sidebarOpen ? 'hidden' : 'flex'}`}
      ...
      {/* Overlay semi-transparent quand le panneau est ouvert */}
      {sidebarOpen && (
        <div 
      ...
      <h1 className="text-4xl font-bold mb-6 text-black">Référentiels AICN</h1>
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {Object.entries(groupedFields).map(([groupName, fields], groupIndex) => (
                    <React.Fragment key={`${entity['entity-id']}-${groupName}`}>
                      ...
                      >
                        <td colSpan={5} className="px-6 py-4 text-indigo-800">
                          <div className="flex items-center w-full justify-between">
                            <div className="flex items-center">
                              ...
                              <h3 className="text-base font-bold">{groupName}</h3>
                              
                              {/* Indicateur de conversations pour le groupe */}
                              {(() => {
                          ...
                          >
                            <td className="px-6 py-4 text-sm text-gray-500 text-center font-mono">
                              {isFieldSelected(entity['entity-id'], field['id-field']) && (
                                <div className="inline-flex justify-center items-center w-5 h-5 mr-2 rounded-full bg-blue-500 text-white">
                                  ...
                                  <div className="ml-3 flex flex-wrap space-x-1">
                                    {fieldConversations.map(conv => (
                                      <button
                                        ...
                                        className="flex items-center rounded-full bg-blue-100 hover:bg-blue-200 text-blue-800 text-xs px-2 py-0.5 mb-1"
                                      >
                                        <svg className="h-3 w-3 mr-1" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                                          <path fillRule="evenodd" d="M18 10c0 3.866-3.582 7-8 7a8.841 8.841 0 01-4.083-.98L2 17l1.338-3.123C2.493 12.767 2 11.434 2 10c0-3.866 3.582-7 8-7s8 3.134 8 7zM7 9H5v2h2V9zm8 0h-2v2h2V9zM9 9h2v2H9V9z" clipRule="evenodd" />
                                        </svg>
